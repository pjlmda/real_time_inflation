...
- **Priorities after France, in this order**: Germany, then the UK, then the US — per the bottleneck analysis in [docs/future-roadmap.md](docs/future-roadmap.md) Part 2. Germany is the cheapest next step methodologically (still an EU member, so `weights/eurostat.py` needs only a `geo=DE` change, no new code — confirmed live) but is currently **shelved** on data-source grounds — see next bullet. The UK needs a new ONS-based weights fetcher (no longer reports into Eurostat post-Brexit) — not yet researched, deliberately skipped for now. The US ended up moving fastest of the three despite being nominally last in this order — see next bullet.
- **Germany — shelved, no viable data source found.** Twelve chains checked live (2026-07-11): Edeka/REWE/Kaufland/Aldi Süd/Netto (both) are bot-blocked (Akamai/Cloudflare); Aldi Nord/Norma/Globus/tegut are accessible but have no independent, priced online grocery catalog; Penny has real prices but no stable per-product URLs (JS-only overlay tiles, 404 on direct navigation) and is a weekly-rotating circular, not a persistent catalog. Lidl Germany looked like the strongest lead (same commerce platform as Lidl France, not bot-blocked, `scraper/lidl_france.py`'s bug fixes reproduced identically) but was ruled out after confirming its online shop sells only non-food "Aktionsware" and wine/beer/spirits — no real day-to-day groceries, across 39 search terms and direct navigation to its own nav-confirmed food-adjacent hub pages. Full writeup, including what would need to change to revisit this, in [docs/germany-expansion-plan.md](docs/germany-expansion-plan.md). No `scraper/lidl_germany.py` or any other DE scraper exists.
- **US — third country, Wegmans built and live-verified across four locations, merged to `main`.** Seventeen US chains checked live (2026-07-11): the top 4 by market share (Walmart, Kroger, Costco, Target) are all confirmed behind enterprise-grade bot mitigation; Albertsons/Safeway/Trader Joe's/Meijer/WinCo/H-E-B are also confirmed blocked. Aldi US has a real, priced catalog but its own `robots.txt` disallows all crawling, ruling it out by this project's own policy regardless of technical scrapability. **Wegmans** (`scraper/wegmans.py`) is genuinely open — cleanest `robots.txt` of the 17, zero enterprise bot-mitigation cookies — and is a real, working scraper with a deliberately deep pilot basket (58 products across 14 categories, ~4.1/category — denser than any prior first-pass basket, per explicit instruction that the US's much larger population needs more products per class to be representative). **Prices vary substantially by store location, confirmed live** — $2.99/gallon milk at Medford, NY vs. $3.99/gallon in Manhattan, the same class of finding as Auchan France's Paris-vs-Marseille discovery — so `scraper/wegmans.py` was rebuilt on `api.digitaldevelopment.wegmans.cloud`'s public JSON commerce API (the site's own frontend calls it unauthenticated; `robots.txt` 404s there) rather than DOM-scraping, since it takes a `storeNumber` parameter directly — no session/UI automation needed to track multiple locations. Four tracked now: `wegmans-us-medford`, `wegmans-us-nyc`, `wegmans-us-fairfax` (VA), `wegmans-us-chapelhill` (NC, added per explicit follow-up instruction to add a location as geographically distant as possible from the first two states — the southernmost point in Wegmans' whole footprint); all three new locations ran for real (NYC 55/58, Fairfax 54/58, Chapel Hill 55/58 — a small, overlapping set of products genuinely not carried at those specific stores, not a bug). On weights: BLS's R-COICOP/R-HICP research series are a real, official COICOP/HICP crosswalk (refreshed every January, same cadence as Eurostat) — correcting this file's earlier claim that no such crosswalk exists. `weights/bls.py` is built too, using the same free unauthenticated `api.bls.gov` JSON endpoint with `"aspects": true` — 13 of 15 mapped item codes individually live-verified, 2 disclosed gaps (BLS doesn't split rice/pasta or break out olive oil/wine; no yoghurt-specific item exists). **Done, 2026-07-12: `category_weights` has 14 real US rows.** A genuine bug beyond the earlier-flagged quota exhaustion had to be fixed first — `fetch_weights()` was sending bare BLS item codes (`SEFA03`) as series IDs instead of the full `CUUR0000SEFA03` form BLS's API actually requires, so requests came back "Invalid Series" even once the quota (raised via a user-registered `BLS_API_KEY`, 500 req/day vs. 25 unregistered) stopped blocking them. `metrics.compute`/`metrics.category_compute` re-run manually for the two most recent days once weights landed — the US now has a real headline index and appears in the live dashboard's country switcher automatically. Several real bugs found and fixed along the way (a hardcoded `currency='EUR'` in `scraper/db.py`, a hardcoded `SOURCE_DATASET` in `weights/eurostat.py:upsert_weights()`, a silently-successful rate-limited weights fetch, a client-side price-hydration timing issue, plus the series-ID bug above) — full detail in [docs/us-expansion-plan.md](docs/us-expansion-plan.md) and `seed/README.md`'s "United States: Wegmans" section, not repeated here. Live promo/regular-price detection remains unconfirmed (no promoted product found in research). **Merged to `main` 2026-07-11, and added to `scrape.yml`'s scheduled matrix the same day** — all four locations now scrape twice daily alongside every other store, per explicit instruction once the basket proved stable across all four real runs.
...
- `web/api/db.py` takes `country` as a per-request param on every `SupabaseReader` method (no more module-level `ACTIVE_COUNTRY` constant) — the live dashboard has a `CountrySwitcher` dropdown (`web/app/components/CountrySwitcher.tsx`) backed by `GET /api/countries`, which lists only countries with real `inflation_metrics` rows, so a new country appears automatically once its data lands, no frontend change needed.
...
- [docs/germany-expansion-plan.md](docs/germany-expansion-plan.md) — Germany-specific research: market-share data, live anti-bot findings across 12 chains, why Lidl Germany (the strongest lead) turned out not to sell groceries online. **Shelved** — read before restarting any Germany work so this research isn't repeated.
- [docs/us-expansion-plan.md](docs/us-expansion-plan.md) — US-specific research and build status: market-share data, live anti-bot findings across 17 chains, a correction to this file's earlier "no clean COICOP crosswalk" claim (BLS does publish one), and `scraper/wegmans.py`'s build writeup (58 products across 3 locations, several real bugs found and fixed). Merged to `main`; `category_weights` now has real US rows and `inflation_metrics` has real `overall`/`store` data — the US is live on the dashboard's country switcher (§10 covers the series-ID bug in `weights/bls.py` that had to be fixed first).
