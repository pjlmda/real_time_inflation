...
- **US — research done, not started, not shelved.** Sixteen chains checked live (2026-07-11): the top 4 by market share (Walmart, Kroger, Costco, Target) are all confirmed behind enterprise-grade bot mitigation (an explicit "Robot or human?" challenge, a connection-level rejection, Akamai, and PerimeterX respectively); Albertsons/Safeway/Trader Joe's/Meijer/WinCo are also blocked. Aldi US is not technically bot-blocked and has a real, priced, location-aware online catalog — but its own `robots.txt` ends with a catch-all `Disallow: /` for all but a few named crawlers, which rules it out under this project's "respect robots.txt" rule regardless of technical scrapability. H-E-B and Wegmans both have permissive `robots.txt` and passed a basic reachability check but weren't conclusively confirmed either way — the most promising open thread, not a dead end. On weights: BLS's R-COICOP/R-HICP research series are a real, official COICOP/HICP crosswalk (refreshed every January, released by end of February, same cadence as Eurostat) — correcting this file's earlier claim that no such crosswalk exists — but `bls.gov`'s own site (hosting the weight `.xlsx` downloads) is Akamai-blocked, `api.bls.gov`'s public JSON API is separately confirmed live and open but its weights-granularity wasn't confirmed, and R-COICOP's own publication was paused in December 2024 with no resume date. Full writeup in [docs/us-expansion-plan.md](docs/us-expansion-plan.md); nothing shelved, more investigation (H-E-B/Wegmans depth, the BLS weights-API question) needed before a build-or-shelve call.
