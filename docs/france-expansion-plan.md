...
2. **Phase 1 — Auchan France spike: done, hypothesis rejected, path forward found.** Live Playwright verification (2026-07-09, see above) confirmed Auchan France isn't bot-blocked but needs entirely new selectors, plus a genuinely new mechanic (delivery-zone selection before any price is visible) — resolved by standardizing on "Livraison à domicile" for one reproducible per-city price rather than picking among many drive points. Remaining before real data collection: finish the delivery-zone-confirmation selector sequence, then build `AuchanFranceScraper` and curate a small pilot basket (~10–15 products across 3–4 categories) end-to-end through the pipeline before committing to full basket curation.
