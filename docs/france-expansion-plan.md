...
2. **Phase 1 — Auchan France spike: done. Scraper built and wired end-to-end, 2026-07-09.** `scraper/auchan_france.py` implements the full confirmed flow (session-level Drive-location confirmation via a `_build_context` override, price extraction via `[itemprop='price']` microdata, price-per-unit via a text-pattern match since that element has no distinguishing class). Registered in `scraper/run.py`'s `SCRAPERS`, config added to `config/stores.yaml` (`auchan-fr-paris`, `auchan-fr-marseille`, both `active: false` until a basket exists), both seeded into the live `stores` table with `country='FR'`, confirmed via `python -m scraper.run --store auchan-fr-paris --mode basket --dry-run`. Along the way, fixed a real bug in `seed/stores.py:load_store_rows()` — it hardcoded `country="PT"` for every store regardless of `config/stores.yaml`, which would have silently mis-seeded these as Portuguese. 102 tests passing.
   ...
   - **Next**: curate a small pilot basket (~10–15 products across 3–4 categories) for both locations, end-to-end through the real (non-dry-run) pipeline, before committing to a full basket.
