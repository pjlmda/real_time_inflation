...
- **(Resolved, 2026-07-12) Redundant query pattern in `metrics/compute.py`.** `compute_metrics_for_date` used to fetch *all* active listings across every store (the "overall" scope), then loop over each store and fetch that store's listings again (a strict subset) for the "store" scope. Fixed by partitioning the one already-fetched `overall_rows`/`snapshots_by_listing` by `store_id` in Python instead of re-querying — `_compute_scope` now takes a pre-fetched snapshots map as a parameter rather than fetching internally.
- **(Resolved, 2026-07-12) No batched writes / one query per (scope, period) for lookback rates.** `_write_index_and_rates` (in both `metrics/compute.py` and `metrics/category_compute.py`) used to issue one `_existing_index` read + one upsert per period, per scope — up to ~9 round trips per scope, and hundreds-to-thousands per compute run across a country's categories/stores. Fixed via two new batched reads per country+family — `fetch_lookback_indices` (all 4 periods' lookback dates fetched in one `as_of_date IN (...)` query, since every scope shares the same 4 target dates) and `fetch_recent_daily_map` (one 6-day-range query covering every scope's moving-average history) — plus a single bulk `upsert()` of every row for that country at the end of the run, replacing the per-row upserts. Verified behaviorally identical against real production data via a read-only dry run (0 mismatches against already-computed PT/category_avg rows). `web/api/db.py`'s `get_health()`/`get_stores()` had the same per-store-per-mode N+1 pattern (up to ~11 sequential round trips for one `/api/health` call) — fixed the same way, one batched `scrape_runs` fetch grouped in Python instead of a query per store/mode. `get_categories()`'s per-category base-date lookup used to scan the *entire* history of every category's daily rows on every call (unbounded growth as history accrues); fixed by bounding it to one small `.limit(1)` query per category instead — category count is small and stable, unlike total history.
