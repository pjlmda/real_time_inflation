...
2. **Phase 1 — Auchan France: done, real data flowing, 2026-07-10.** `scraper/auchan_france.py` implements the full confirmed flow (session-level Drive-location confirmation via a `_build_context` override, price extraction via `[itemprop='price']` microdata, price-per-unit via a text-pattern match since that element has no distinguishing class). Registered in `scraper/run.py`'s `SCRAPERS`; `auchan-fr-paris`/`auchan-fr-marseille` both `active: true` in `config/stores.yaml` and seeded into the live `stores` table with `country='FR'` (not yet in `.github/workflows/scrape.yml`'s matrix — manual-run only for now). Along the way, fixed a real bug in `seed/stores.py:load_store_rows()` — it hardcoded `country="PT"` for every store regardless of `config/stores.yaml`, which would have silently mis-seeded these as Portuguese.
   ...
   **Pilot basket curated and scraped live, grown to 11 products across 11 categories** (rice, bread, pasta, poultry, yoghurt, cheese, eggs, olive oil, fresh fruit, vegetables, personal care — real EANs pulled from each PDP's "Réf / EAN" field), each with a listing at both locations. `python -m scraper.run --store auchan-fr-paris/-marseille --mode basket` both ran for real: **11/11 listings, 100% coverage, at both locations.** Confirmed genuine regional price variation on the same day — e.g. rice 2.24€ in Paris vs. 2.10€ in Marseille, eggs 1.76€ vs. 1.65€, carrots 2.99€ vs. 3.99€. The real `metrics/compute.py` job was also run for real: 64 France-only `inflation_metrics` rows produced, zero Portugal rows in the same run (confirmed correct — Portugal's own scrape hadn't landed yet that run, and this is exactly the isolation the country-scoping fix in Phase 0 was built to guarantee).
   - **Not yet confirmed live**: promo/regular-price detection (`is_promotion`/`regular_price` currently default to "no promotion" — no promoted product was encountered during research on Auchan specifically; Lidl France's basket below did confirm this pattern works when a real promo is live).
   - **Next**: grow the basket further (8 more categories to match Portugal's 19), add these two stores to `scrape.yml`'s matrix once satisfied with stability.
3. **Phase 2 — Lidl France: done, real data flowing including live-verified promo detection, 2026-07-11.** Confirmed live via real Playwright sessions: not bot-blocked, and — unlike Auchan France — prices show directly with no delivery-zone confirmation step, a single national price like the Portuguese stores. Product tiles expose structured JSON via a `data-gridbox-impression` attribute (id/name/brand/price, used for curation); PDPs use clean, stable price-block classes (`.ods-price__value`, `.ods-price__stroke-price` for the pre-promo price when active, `.ods-price__footer` for package size and price-per-unit). `scraper/lidl_france.py` implements this. EAN wasn't reliably reachable (buried in a Nuxt-flattened SSR JSON payload not worth reverse-engineering for a pilot basket) — listings use `match_method='manual'`/`ean='TODO'`, the same convention already used elsewhere in this project when an EAN isn't available.

   Two real bugs found and fixed during this build, both would have affected every future Lidl scrape (France or Germany) deterministically, not just occasionally:
   - `scraper/antibot.py:RobotsChecker` used stdlib `urllib`/`ssl` to fetch `robots.txt`, which can't complete the certificate chain some CDNs present (confirmed live for Lidl's `myracloud` CDN) even though the exact same domain works fine via a real browser or via `httpx` (which uses `certifi`'s more complete trust store). Fixed by fetching via `httpx` instead, preserving the same status-code semantics stdlib `robotparser` already had (401/403 disallow all, other 4xx allow all, 2xx parse normally).
   - `scraper/antibot.py:detect_block()` false-triggered on every single Lidl page — Lidl's Nuxt frontend ships a `friendlyCaptchaSitekey` config value in an embedded `<script>` JSON blob on every page load (for a form elsewhere on the site), which matched the generic `"captcha"` block-marker even though nothing was actually blocked. Fixed in `scraper/lidl_france.py` by checking rendered visible text (`page.inner_text("body")`) instead of raw HTML — a real block/CAPTCHA page's markers are meant to be human-readable text, never JS config values.

   **Pilot basket curated and scraped live**: 12 products across 12 categories (bread, pasta, beef, pork, poultry, dried/smoked fish, preserved fish, yoghurt, cheese, olive oil, wine, personal care). Rice was attempted but not found in Lidl France's current online catalog/search index despite multiple search attempts — likely a genuine gap in Lidl's smaller, rotating online assortment compared to a full hypermarket like Auchan, not a scraper limitation. `python -m scraper.run --store lidl-fr --mode basket` ran for real: **12/12 listings, 100% coverage.** 8 of the 12 products were on a genuine live promotion at curation time (e.g. cheese -30%, personal care -30%, wine -50%), giving real, confirmed end-to-end verification of promo/regular-price detection — something Auchan France's basket never had the chance to confirm.
...
5. **Phase 3 — full basket curation** for both stores, following the same category-by-category, live-verified process as every PT basket-growth round (still open: 8 more categories to match Portugal's 19).
