...
- **US — third country, Wegmans built and live-verified, on a research branch only.** Seventeen US chains checked live (2026-07-11): the top 4 by market share (Walmart, Kroger, Costco, Target) are all confirmed behind enterprise-grade bot mitigation; Albertsons/Safeway/Trader Joe's/Meijer/WinCo/H-E-B are also confirmed blocked. Aldi US has a real, priced catalog but its own `robots.txt` disallows all crawling, ruling it out by this project's own policy regardless of technical scrapability. **Wegmans** (`scraper/wegmans.py`) is genuinely open — cleanest `robots.txt` of the 17, zero enterprise bot-mitigation cookies, real UPCs embedded in page data (better barcode coverage than either Lidl France or Lidl Germany managed) — and is now a **real, working scraper**, not just a lead: a deliberately deep pilot basket (58 products across 14 categories, ~4.1/category — denser than any prior first-pass basket, per explicit instruction that the US's much larger population needs more products per class for the within-class average to be representative) ran for real at **58/58 listings, 100% coverage**. Building it surfaced a real, general bug — `scraper/db.py` hardcoded `"currency": "EUR"` for every store unconditionally, never caught before because every store built so far has been EUR-denominated; fixed by adding `currency` to `StoreConfig`/`config/stores.yaml` the same way `timezone_id` already was. On weights: BLS's R-COICOP/R-HICP research series are a real, official COICOP/HICP crosswalk (refreshed every January, same cadence as Eurostat) — correcting this file's earlier claim that no such crosswalk exists. `weights/bls.py` is now built too: the same unauthenticated `api.bls.gov` JSON endpoint, with `"aspects": true` added to the request body, freely returns current relative-importance (weight) figures for any CPI item series, no registration or key needed — 13 of 15 mapped item codes were individually live-verified (2 sourced from search+FRED cross-reference only, flagged in the module). Two disclosed gaps mirror this project's existing "real gap, not worked around" pattern: BLS doesn't split rice from pasta or break out olive oil/wine from broader categories, and no yoghurt-specific item was found at all. Building it surfaced two more real bugs: `weights/eurostat.py:upsert_weights()` hardcoded its own `SOURCE_DATASET` constant regardless of caller (would have mislabeled every BLS-sourced audit row as Eurostat's) — fixed with a `source_dataset` parameter; and `weights/bls.py` itself initially let a rate-limited API response silently produce a "Synced 0 weight records" success message instead of failing loudly — fixed by checking the response's own status field. **`category_weights` has no US rows yet** — the daily unauthenticated request quota was exhausted by this same session's research before a full live sync could run; needs a re-run once the quota resets before US `inflation_metrics` can be computed at all. Live promo/regular-price detection on the scraper side and whether Wegmans' default store location is a fixed default or IP-geolocation-based both also remain open. Full writeup in [docs/us-expansion-plan.md](docs/us-expansion-plan.md) and `seed/README.md`'s "United States: Wegmans" section. **Everything here lives on the `research/us-expansion` branch — not merged to `main`, not in `scrape.yml`'s scheduled matrix**, per explicit instruction to keep iterating there before any merge decision.
...
- [docs/us-expansion-plan.md](docs/us-expansion-plan.md) — US-specific research and build status: market-share data, live anti-bot findings across 17 chains, a correction to this file's earlier "no clean COICOP crosswalk" claim (BLS does publish one), and `scraper/wegmans.py`'s build writeup (58 products, 3 real bugs found and fixed, 58/58 live-verified). On `research/us-expansion` only, not merged to `main` — read before continuing any US work.
