...
- **US — research done, not started, not shelved. Wegmans is a real, live-confirmed lead.** Seventeen chains checked live (2026-07-11): the top 4 by market share (Walmart, Kroger, Costco, Target) are all confirmed behind enterprise-grade bot mitigation (an explicit "Robot or human?" challenge, a connection-level rejection, Akamai, and PerimeterX respectively); Albertsons/Safeway/Trader Joe's/Meijer/WinCo/H-E-B are also confirmed blocked (H-E-B's apparent "blank page" turned out to be an explicit anti-automation JSON error page on closer inspection). Aldi US is not technically bot-blocked and has a real, priced, location-aware online catalog — but its own `robots.txt` ends with a catch-all `Disallow: /` for all but a few named crawlers, which rules it out under this project's "respect robots.txt" rule regardless of technical scrapability. **Wegmans is genuinely open**: cleanest `robots.txt` of any of the 17 chains (unconditional `Allow: /`), zero enterprise bot-mitigation cookies across a full session, real structured prices *and* price-per-unit, stable PDP URLs, and a real UPC embedded in page data (better barcode access than either Lidl France or Lidl Germany got) — the same shape of finding Lidl was for France/Germany, just for a smaller, Northeast-US-only regional chain. On weights: BLS's R-COICOP/R-HICP research series are a real, official COICOP/HICP crosswalk (refreshed every January, same cadence as Eurostat) — correcting this file's earlier claim that no such crosswalk exists. `bls.gov` (the weight `.xlsx` downloads) and `download.bls.gov` (BLS's separate bulk flat-file service) are both confirmed Akamai-blocked — the latter's block page explicitly states BLS's own anti-automation policy in writing — but **the weights-access question is fully resolved**: the same unauthenticated `api.bls.gov` JSON endpoint, with `"aspects": true` added to the request body, freely returns current, correctly-structured relative-importance (weight) figures for any CPI item series, no registration or API key needed, confirmed live at both broad-aggregate and fine-grained item level. The one remaining nuance is whether that's BLS's *native* item-strata weighting or the already-COICOP-bridged output specifically (the bridged product may still only exist in the paused/blocked files) — but either way this is now a bounded mapping task, not a missing data source. Full writeup in [docs/us-expansion-plan.md](docs/us-expansion-plan.md); nothing shelved, nothing built — next concrete step is a real DOM/pricing spike on Wegmans, the same discipline every other store in this project got before a scraper was written.
...
- [docs/us-expansion-plan.md](docs/us-expansion-plan.md) — US-specific research: market-share data, live anti-bot findings across 17 chains (top 4 all blocked, Aldi US ruled out by its own `robots.txt`, Wegmans confirmed genuinely open), and a correction to this file's earlier "no clean COICOP crosswalk" claim — BLS does publish one (R-COICOP/R-HICP), with its own real access caveats. Research only, not shelved — read before continuing any US work.
